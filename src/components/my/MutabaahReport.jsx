  ...
  const printReport = () => {
    ...
    printWindow.document.write(`
      ...
      <head>
        ...
        <title>Laporan Mutaba'ah Yaumiyah</title>
        <style>
      ...
      <body>
        <div class="header">
          ...
          <p>${user?.name || '-'}</p>
          <p>Periode: ${months.find(m => m.value === month)?.label} ${year}</p>
        ...
        <table class="info-table">
          <tr>
            ...
            <td width="20%"><strong>Total Hari</strong></td>
            <td width="30%">${allUserData.length}</td>
          </tr>
          <tr>
            <td><strong>Periode</strong></td>
            <td>${months.find(m => m.value === month)?.label} ${year}</td>
        ...
        <table class="main-table">
          <thead>
            <tr>
              ...
              <th>Haid</th>
              <th>Tilawah Quran</th>
              <th>Terjemah Quran</th>
              <th>Dzikir Pagi/Petang</th>
              <th>Istighfar 1000x</th>
              <th>Sholawat 100x</th>
              <th>MQ Pagi</th>
            </tr>
          </thead>
          <tbody>
            ${allUserData.map((data, index) => `
              <tr>
                ...
                <td>${formatHijriDate(data.date)}</td>
                <td>${data.sholat_wajib}/5${data.haid > 0 ? ' (Haid)' : ''}</td>
                <td>${data.sholat_tahajud > 0 ? '✓' : '✗'}</td>
                <td>${data.sholat_dhuha > 0 ? data.sholat_dhuha + ' rakaat' : '✗'}</td>
                <td>${data.shaum_sunnah > 0 ? '✓' : '✗'}</td>
                <td>${data.haid > 0 ? '✓' : '✗'}</td>
                <td>${data.tilawah_quran > 0 ? '✓' : '✗'}</td>
                <td>${data.terjemah_quran > 0 ? '✓' : '✗'}</td>
                <td>${data.dzikir_pagi_petang > 0 ? '✓' : '✗'}</td>
                <td>${data.istighfar_1000x > 0 ? '✓' : '✗'}</td>
                <td>${data.sholawat_100x > 0 ? '✓' : '✗'}</td>
                <td>${data.menyimak_mq_pagi > 0 ? '✓' : '✗'}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
        ...
        <div class="stats-container">
          ...
          <div class="stats-grid">
            <div class="stat-box">
              <div class="stat-title">Sholat Wajib (Rata-rata)</div>
              <div class="stat-value">${stats.avgSholatWajib}/5</div>
            </div>
            <div class="stat-box">
              <div class="stat-title">Tahajud (Hari)</div>
              <div class="stat-value">${stats.tahajudDays}/${allUserData.length}</div>
            </div>
            <div class="stat-box">
              <div class="stat-title">Dhuha (Hari)</div>
              <div class="stat-value">${stats.dhuhaDays}/${allUserData.length}</div>
            </div>
            <div class="stat-box">
              <div class="stat-title">Shaum Sunnah (Hari)</div>
              <div class="stat-value">${stats.shaumDays}/${allUserData.length}</div>
            </div>
            <div class="stat-box">
              <div class="stat-title">Haid (Hari)</div>
              <div class="stat-value">${stats.haidDays}/${allUserData.length}</div>
            </div>
            <div class="stat-box">
              <div class="stat-title">Tilawah Quran (Hari)</div>
              <div class="stat-value">${stats.tilawahDays}/${allUserData.length}</div>
            </div>
            <div class="stat-box">
              <div class="stat-title">Terjemah Quran (Hari)</div>
              <div class="stat-value">${stats.terjemahDays}/${allUserData.length}</div>
            </div>
            <div class="stat-box">
              <div class="stat-title">Istighfar 1000x (Hari)</div>
              <div class="stat-value">${stats.istighfarCompleted}/${allUserData.length}</div>
            </div>
            <div class="stat-box">
              <div class="stat-title">Sholawat 100x (Hari)</div>
              <div class="stat-value">${stats.sholawatCompleted}/${allUserData.length}</div>
            </div>
            <div class="stat-box">
              <div class="stat-title">MQ Pagi (Hari)</div>
              <div class="stat-value">${stats.mqDays}/${allUserData.length}</div>
            </div>
          </div>
        </div>

        <div class="footer">
          <p>Dicetak dari Sistem Mutaba'ah Yaumiyah pada ${new Date().toLocaleString('id-ID')}</p>
        </div>
